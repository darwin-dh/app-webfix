<script >
import Swal from "sweetalert2";
import axios from "axios";
import router from "@/router";
export default {
  props: {
    items: {
      type: Object,
    },
  },
  methods: {
    see(items) {
      console.log("ey", items.idformapago);
    },
    edit(items) {
      router.push({
        name: "EditarFormaPagos",
        params: { id: items.idformapago },
      });
    },
    async delet(items) {
      try {
        const result = await axios.post("api/borrar-formapago", {
          idformapago: items.idformapago,
        });
        const data = result.data.data;
        if (data === "YES")
          Swal.fire({
            icon: "success",
            title: "Se borro con exito",
            showConfirmButton: false,
            timer: 1500,
          });
        this.$emit("getPays");
      } catch (error) {
        Swal.fire({
          icon: "error",
          title: error.response.data.error,
          showConfirmButton: false,
          timer: 1500,
        });
      }
    },
  },
};
</script>
<template>
  <tr
    v-for="(data, index) in items"
    :key="index"
    class="rounded-pill mb-5 py-5"
  >
    <td class="">
      {{ data.idproductos  }}
    </td>
    <td class="project_name">
      {{ data.codigo }}
    </td>
    <td class="project_name">
      {{ data.nombre }}
    </td>
    <td class="project_name">
      {{ data.codigoAlterno }}
    </td>
    <td class="project_name">
      {{ data.codigo_barras }}
    </td>
    <td class="project_name">
      {{ data.caracteristicas }}
    </td>
    <td>{{data.ubicacion_fisica}}</td>
    <td>
      <span
        class="badge text-uppercase"
        :class="{
          'bg-danger': data.estado == 'inactivo',
          'bg-success': data.estado == 'activo',
        }"
        >{{ data.estado }}</span
      >
    </td>
    <td>{{ data.unidad_entrada }}</td>
    <td>{{ data.existencia_maxima }}</td>
    <td>{{ data.existencia_minima }}</td>
    <td>{{ data.numero_unidad }}</td>

    <td>
      <div class="d-flex align-items-center acciones">
        <button
          class="btn btn-outline-info d-flex align-items-center text-info px-2"
          @click="see(data)"
        >
          <i class="mdi mdi-eye mr-2"></i>
          Ver
        </button>

        <button
          class="
            btn btn-outline-warning
            d-flex
            align-items-center
            text-warning
            px-2
          "
          @click="edit(data)"
        >
          <i class="mdi mdi-pencil-box-outline"></i> Editar
        </button>
        <button
          class="
            btn btn-outline-danger
            d-flex
            align-items-center
            text-danger-900
            px-2
          "
          @click="delet(data)"
        >
          <i class="mdi mdi-trash-can-outline"></i>Borrar
        </button>
      </div>
    </td>
  </tr>
</template>